                ...
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-col-6 gap-6">
                    @foreach ($slides as $slide)
                        <div
                            ...
                            <img src="{{ asset('uploads/' . $slide->img) }}" alt="{{ $slide->name_img }}"
                                class="w-full h-40 object-contain">
                            <div class="p-6">
                                <h3 class="text-xl font-bold text-gray-900">{{ $slide->name_img }}</h3>
                            </div>
                        </div>
                    @endforeach
                </div>
                    ...
                    @foreach ($products as $product)
                        <div
                            ...
                            @if (!empty($product->img))
                                <img src="{{ asset('uploads/' . $product->img) }}" alt="{{ $product->name_product }}"
                                    class="w-full h-auto">
                            @else
                                <img src="https://p7.hiclipart.com/preview/696/451/637/computer-icons-inventory-business-management-warehouse-warehouse.jpg" alt="Default Image" class="w-full h-auto">
                            @endif

                            class="w-full h-48 object-cover">
                            <div class="p-6">
                                <h3 class="text-xl font-bold text-gray-900">{{ $product->name_product }}</h3>
